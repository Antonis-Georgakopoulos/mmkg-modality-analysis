...
from longdocurl.eval import extract_answer
...
async def answer_with_modality_subset(
    ...
    try:
        ...
        # Parse extracted answer - extract content from <concise_answer> tags
        try:
            # First get the text after "Extracted answer:"
            raw_prediction = extracted_result.split("Answer format:")[0].split("Extracted answer:")[1].strip()
            
            # Then extract content from <concise_answer> tags (keep as-is, no modification)
            concise_match = re.findall(r"<concise_answer>(.*?)</concise_answer>", raw_prediction, re.DOTALL)
            if concise_match:
                prediction = concise_match[0].strip()
            else:
