"""
Forest plots for pooled cluster bootstrap results.

Reads pooled_consistency_results.json and produces single-panel forest
plots (since all benchmarks are merged into one pool).
"""
...
RESULTS_JSON = Path(__file__).parent / "pooled_consistency_results.json"
...
# ── Per-model forest plot (single panel, pooled) ─────────────────────────────
...
def plot_forest_permodel(pairs, metric, output_path):
    ...
    if metric == "D":
        title = "Pooled Cross-Model Contribution Analysis (cluster bootstrap)"
    else:
        title = "Pooled Cross-Model Cooperation Analysis (cluster bootstrap)"
...
def main():
    ...
    for metric in ("D", "C12"):
        path = OUTPUT_DIR / f"pooled_forest_{metric}_permodel.png"
