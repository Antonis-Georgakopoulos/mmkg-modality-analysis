"""
Forest plot for stratified question-level bootstrap results.
"""
...
RESULTS_JSON = Path(__file__).parent / "stratified_results.json"
...
def plot_forest_permodel(pairs, metric, output_path, title_prefix="Stratified"):
...
def main():
    with open(RESULTS_JSON, "r", encoding="utf-8") as f:
        raw = json.load(f)
    # Handle both old (flat) and new (nested) JSON format
    all_results = raw.get("per_pair", raw)

    for metric in ("D", "C12"):
        path = OUTPUT_DIR / f"stratified_forest_{metric}_permodel.png"
        plot_forest_permodel(all_results, metric, path,
                             title_prefix="Stratified Bootstrap")
